...
def checkin():
    ...
    if trenutno_datum_vrijeme < svi_konkretni_letovi[sve_karte[broj_karte]['sifra_konkretnog_leta']]['datum_i_vreme_polaska']:
        ...
        unesite_bilo_koji_karakter()
    p = True           
    if kor_ime == sve_karte[broj_karte]['kupac']['korisnicko_ime']: #ako je ulogovani korisnik kupac
        while(p):
            lozinka = input('\nDa bi potvrdili indentitet kupca karte molimo unesite Vasu lozinku:')
            if sve_karte[kor_ime]['lozinka'] != lozinka:
                print('Pogresna lozinka pokusajte ponovo!')
            else:
                p = False
    while not p:        #ako je iznad tacno p je false
        ime = input('\nUnesite ime kupca: ')
    ...
    if len(sve_karte[broj_karte]['kupac']) == 4:
        while(t):
            korisnicko_ime = input('Za dalju potvrdu potrebne su nam dodatne informacije:\nIzaberite korisnicko ime: ')
...
def pretraga_karata():
    ...
    korisnikovo_ime = input('Unesite korisnicko ime kupca: ')
    try:
        if datum_polaska != '':
        ...
        unesite_bilo_koji_karakter()
    lista = karte.pretraga_prodatih_karata(sve_karte, svi_letovi, svi_konkretni_letovi, polaziste, odrediste, datum_polaska, datum_dolaska, korisnikovo_ime)
    if len(lista) == 0:
...
def kupovina_karata():
    ...
    karta, sve_karte = karte.kupovina_karte(sve_karte, svi_konkretni_letovi, sifra, putnici, svi_konkretni_letovi[sifra]["zauzetost"], kupac, prodavac = {"id": 1, "uloga": konst.ULOGA_PRODAVAC}, datum_prodaje = date.today())
    karte.sacuvaj_karte(sve_karte, 'karte.csv', ',')
    if uloga_kor == konst.ULOGA_PRODAVAC:
    ...
    if uloga_kor == konst.ULOGA_PRODAVAC:
        prodavac = {"kor_ime": kor_ime, "uloga": konst.ULOGA_PRODAVAC}
    else:
        prodavac = {"kor_ime": 1, "uloga": konst.ULOGA_PRODAVAC}
    matrica = letovi.matrica_zauzetosti(konkretan)
    karta, sve_karte = karte.kupovina_karte(sve_karte, svi_konkretni_letovi, sifra, putnici, svi_konkretni_letovi[sifra]["zauzetost"], kupac, prodavac, datum_prodaje = date.today())
    karte.sacuvaj_karte(sve_karte, 'karte.csv', ',')
    if uloga_kor == konst.ULOGA_PRODAVAC:
        print("Uspjesno prodata karta!")
    else:
...
def izvjestavanje():
    ...
    try:
        ...
        if izbor == '*':
            uvodna_poruka()
        izbor = int(izbor)
        if izbor not in range(1, 8) and izbor != '*':
            raise Exception
        if izbor == 1: 
            clear()
            ...
            try:
                for karta in sve_karte.values():
                    dan_polaska = str(svi_konkretni_letovi[karta['sifra_konkretnog_leta']]['datum_i_vreme_polaska'].date())
                    if dan_polaska == datum:
                        broj += 1
            ...
            clear()
            print("_"*50 + '\n\n' + ' '*10 + 'Lista prodanih za dan polaska' + '\n' + "_"*50 + '\n')
            sve_karte_1 = sve_karte
            for karta in sve_karte_1.values():
                karta['prodavac'] = karta['prodavac']['kor_ime']    #test prosljedjuje funkciji prodavca kao string kor ime a ne kao dict
            rjecnik = {}
            try:
                rjecnik = izvestaji.izvestaj_ubc_prodatih_karata_30_dana_po_prodavcima(sve_karte_1, svi_konkretni_letovi, svi_letovi)
            except:
                izvjestaji_pogresan_unos("Pogresan unos")
            if len(rjecnik) == 0:
                izvjestaji_pogresan_unos("Ovaj prodavac nije prodao nijednu kartu.")
            print('\n    Broj karti' + ' '*7 + 'Prodavac' + ' '*4 + 'Ukupna cijena\n' + '-'*50)
            for i in rjecnik:           #{'prodavac':[br, cijena, 'prodavac']}
                lista = rjecnik[i]
                c = 7
                if lista[0] >9:
                    c = 6
                print(' '*c, lista[0],' '*10,i,' '*5, lista[1])
            izvjestaji_pogresan_unos('')
    except Exception as c:
        print(c)
        print('Pogresan unos!')
...
def pregled_nerealizovanih_letova():
    ...
    if len(lista_nerealizovanih) == 0:
        ...
        sleep(3)
        uvodna_poruka()
    print("Br leta" + ' '*4 + 'Datum pocetka i kraja operativnosti' + ' '*8 + 'Cena\n')
